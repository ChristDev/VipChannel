...
{
    public partial class FrmPlanPersonalizadoList : Form
    {
        private PlanPersonalizadoView _planView;
        public FrmPlanPersonalizadoList()
        {
            InitializeComponent();
            Text = FormListNames.FrmPlanPersonalizadoList;
        }

        #region Abrir formulario solo una vez
        private static FrmPlanPersonalizadoList _mFormDefInstance;
        public static FrmPlanPersonalizadoList DefInstance
        {
            get
            {
                if (_mFormDefInstance == null || _mFormDefInstance.IsDisposed)
                    _mFormDefInstance = new FrmPlanPersonalizadoList();
        ...
        private void btnNuevo_Click(object sender, EventArgs e)
        {
            ...
            var form = new FrmPlanPersonalizado(Convert.ToInt32(Operation.Create), id);
        ...
        {
            _planView = new PlanPersonalizadoView();
            vPlanPersonalizadoBindingSource.DataSource = _planView.SelectListView();
        }
        ...
        {            
            if (dgvDatosRegistrados.CurrentRow != null)
            {
                ...
                var form = new FrmPlanPersonalizado(Convert.ToInt32(Operation.Update), id);
                if (DialogResult.OK == form.ShowDialog())
                {
                    CargarDatos();
                }
