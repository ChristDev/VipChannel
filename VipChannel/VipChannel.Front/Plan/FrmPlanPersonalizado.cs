...
{
    public partial class FrmPlanPersonalizado : Form
    {
        ...
        public FrmPlanPersonalizado(int operation, Guid id)
        {
            InitializeComponent();
            _operation = operation;
            _id = id;
            _planType = ConstantPlanType.Personalizado;
        }
        ...
        private void FrmPlanPersonalizado_Load(object sender, EventArgs e)
        {            
            if (_operation == Convert.ToInt32(Operation.Create))
            {
                _planApplication = new PlanApplication();
                lblTituloFormulario.Text = Constants.FormCreateNames.FrmPlanPersonalizadoCreate;
                txtInternalCode.ReadOnly = true;
                txtInternalCode.Text = _planApplication.GenerarCodigoInterno();
            }
            else if (_operation == (int)Operation.Update)
            {
                lblTituloFormulario.Text = Constants.FormCreateNames.FrmPlanPersonalizadoUpdate;
                _planApplication = new PlanApplication();
