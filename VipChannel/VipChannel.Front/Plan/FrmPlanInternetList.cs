...
{
    public partial class FrmPlanInternetList : Form
    {
        private PlanInternetView _planView;
        public FrmPlanInternetList()
        {
            InitializeComponent();
            Text = FormListNames.FrmPlanInternetList;
        }

        #region Abrir formulario solo una vez
        private static FrmPlanInternetList _mFormDefInstance;
        public static FrmPlanInternetList DefInstance
        {
            get
            {
                if (_mFormDefInstance == null || _mFormDefInstance.IsDisposed)
                    _mFormDefInstance = new FrmPlanInternetList();
        ...
        private void btnNuevo_Click(object sender, EventArgs e)
        {
            ...
            var form = new FrmPlanInternet(Convert.ToInt32(Operation.Create), id);
        ...
        {
            _planView = new PlanInternetView();
            vPlanInternetBindingSource.DataSource = _planView.SelectListView();
        }
        ...
        {            
            if (dgvDatosRegistrados.CurrentRow != null)
            {
                ...
                var form = new FrmPlanInternet(Convert.ToInt32(Operation.Update), id);
                if (DialogResult.OK == form.ShowDialog())
                {
                    CargarDatos();
                }
