...
namespace VipChannel.Front.Definitions.Manager
{
    ...
    {
        private Domain.Entity.Manager _managerEntity;
        ...
        private Domain.Entity.Manager SetFormData()
        {
            _managerEntity = new Domain.Entity.Manager()
            {
        ...
        private void btnAsignarZona_Click(object sender, EventArgs e)
        {
            ...
            {
                var id = Guid.Parse(dgvDatosRegistrados.CurrentRow.Cells[0].Value.ToString());
                var employee = dgvDatosRegistrados.CurrentRow.Cells[3].Value.ToString() + " " + dgvDatosRegistrados.CurrentRow.Cells[2].Value.ToString();
                var form = new FrmManagerZone(id, employee);
                form.ShowDialog();
            }
            else
            {
